...
def belief_state(bstate, summary_bstate, domain):
    ...
    for slot in bstate[domain]["semi"]:
        if isinstance(bstate[domain]["semi"][slot], str):
            ...
            not_mentioned = 1 if slot_ == "not mentioned" else 0
            dont_care = 1 if slot_ == "don't care" else 0
            mentioned = 1 if (dont_care == 0 and not_mentioned == 0) else 0

            encoded_state = [not_mentioned, dont_care, mentioned]
            summary_bstate.extend(encoded_state)
...
def requested_state(bstate, summary_bstate, domain):
    ...
    for slot in [
        ...
        # YOUR CODE HERE
        summary_bstate.append(
            1 if slot in bstate[domain]["semi"]["requested"] else 0
        )
        # YOUR CODE ENDS HERE
    return summary_bstate
