...
def naive_softmax_loss_and_gradient(
    ...
    # create binary vector indicating true outside word
    ...
    gradOutsideVecs = np.dot(
        (y_hat - y)[:, np.newaxis], centerWordVec[np.newaxis, :]
    )
...
def neg_sampling_loss_and_gradient(
    ...
    y = sigmoid(np.matmul(outsideWordVec, centerWordVec))
    loss -= np.log(y)

    y_hat = sigmoid(np.matmul(-negSampleWordVecs, centerWordVec))
    loss -= np.sum(np.log(y_hat))

    gradCenterVec = (y - 1) * outsideWordVec
    gradCenterVec -= np.matmul((y_hat - 1).T, negSampleWordVecs)

    # compute weights for loss multiplication
    unique, indices, counts = np.unique(
        negSampleWordIndices, return_index=True, return_counts=True
    )
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] = (y - 1) * centerWordVec
    gradOutsideVecs[unique, :] = -((y_hat[indices] - 1) * counts)[:, np.newaxis]\
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordsIdxs = np.array([word2Ind[word] for word in outsideWords])

    # iterate over all outside words and compute gradients for them
    for idx in outsideWordsIdxs:
        loss_i, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec,
            idx,
            outsideVectors,
            dataset
        )
        # sum loss and gradients for different outside words
        loss += loss_i
