...
class NMT(nn.Module):
    ...
    def encode(
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder.forward(X_packed)
        ...
        ###
        last_hidden = torch.cat(tuple(last_hidden), dim=1)
        init_decoder_hidden = self.h_projection.forward(last_hidden)

        last_cell = torch.cat(tuple(last_cell), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # over. Otherwise, you will remove the batch dimension accidentally, if batch_size = 1.
        ###
        dec_hidden = dec_hidden.unsqueeze(dim=2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden)
        e_t = e_t.squeeze(dim=2)
        dec_hidden = dec_hidden.squeeze(dim=2)
        # Use the following docs to implement this functionality:
