...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # truncate it to a random length
        length = random.randint(4, int(self.block_size*7/8))
        if length > len(document):
            length = len(document)
        document = document[:length]

        # split truncated document into three parts
        masked_content_percent = np.random.normal(0.25, 0.05)
        masked_content_length = int(masked_content_percent * length)
        masked_content_length = int(
            np.clip(masked_content_length, 1, length - 2)
        )
        masked_start_idx = random.randint(1, length - masked_content_length - 1)
        prefix = document[:masked_start_idx]
        masked_content = document[
                         masked_start_idx: masked_start_idx + masked_content_length
                         ]
        suffix = document[masked_start_idx + masked_content_length:]

        # rearrange prefix, masked_content and suffix
        masked_string = (
            f'{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{masked_content}'
        )
        pads = ''.join(
            [self.PAD_CHAR for _ in range(self.block_size - len(masked_string))]
        )
        masked_string = f'{masked_string}{pads}'
        ...
        # encode as tensors
        x = torch.tensor(
            [self.stoi[char] for char in input_string], dtype=torch.long
        )
        y = torch.tensor(
            [self.stoi[char] for char in output_string], dtype=torch.long
        )
